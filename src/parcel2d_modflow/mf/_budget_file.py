"""
This module is taken from imod-python; imod.mf6.out

It makes generating xarray data from the budget file a little easier,
than doing it via Flopy.
"""
...
def open_disgrb(path):
    ...
    coords = _xycoords(bounds, (delc, -delr))
...
def _open_imeth1_budgets(
    ...
    for i, header in enumerate(header_list):
        time[i] = header.totim
        x = _read_imeth1_budgets(cbc_path, header.ndim1, header.pos)
        arr_list.append(x)
...
def _dis_read_imeth6_budgets(
    ...
    with open(cbc_path, "rb") as f:
        f.seek(pos)
        table = np.fromfile(f, dtype, count)
    index = table["id1"] - 1  # Fortran is 1-based, Python is 0-based
    out[index] = table["budget"]
...
def _dis_open_imeth6_budgets(
    ...
    coords["time"] = time
    name = header_list[0].text
    return xr.DataArray(arr, coords, ("time", "layer", "y", "x"), name=name)
...
def read_cbc(cbc_path: FilePath, grb_path: FilePath) -> Dict[str, xr.DataArray]:
    """
    ...
    """
    grb_content = open_disgrb(grb_path)
